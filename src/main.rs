...
impl Status {
    ...
    fn setting_mode(self) -> bool {
        1 & (self.0 >> 3) == 1
    }

    fn cooling_mode(self) -> bool {
        1 & (self.0 >> 2) == 1
    }

    fn manual_mode(self) -> bool {
        1 & (self.0 >> 1) == 1
    }

    fn autotune_mode(self) -> bool {
        1 & self.0 == 1
    }
}
...
impl Syl2381 {
    ...
    fn debug(&mut self) {
        ...
        println!("{: <7} = {}", "AL1_STA", self.get_al1_sta());
        print_holdings!(CV);
        println!("{: <7} = {:#?}", "AT", self.get_at());

        println!();
        println!("Static Params");
        println!("=============");
        print_holdings!(
            SV, AH1, AL1, P, I, D, BB, SOUF, OT, FILT, INTY, OUTY, COTY, HY, PSB, RD, CORF, ID,
            BAUD
        );
    }
    ...
    fn get_coils(&mut self, reg: u16, count: u8) -> u8 {
        ...
        // instead of using mreq.parse_bool, we'll just grab the byte directly
        // TODO: make this work also work for non-RTU
        let val = response[3];
        val

        // This is kinda silly, but I don't know of a better way to pull out the byte from the response.
        // let mut data = Vec::new();
